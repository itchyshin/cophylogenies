...
# testing the new function |--> AICc = 1175.0413
full.model <- updated.rma.mv(yi = Zr, 
                       V = VZr, 
                       mods = ~ symbiosis + 
                         host_tax_broad +
                         symbiont_tax_broad +
...
# model averaging
summary(model.avg(candidate.models,  delta < 2))
...
```{r}
# 
time_lag_effect <- rma.mv(yi = Zr, V = VZr, mods = ~ year, random = ~ 1 | authors, data = dat)
...
```{r}
# getting marginal R2
r2_time_lag_effect <- R2(time_lag_effect)
...
summary(egger_regression_mul)

# residual funnel plot is OK
funnel(egger_regression_mul, yaxis="seinv")
