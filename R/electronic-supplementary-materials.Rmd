...
We have 5 custum functions named : `p_to_Zr()`,`I2()`, `R2()`, `get_est()`, `get_pred()`, and `cont_gen()`, all of which are used later (see below for their functionalites). 
...
We present the number of sample size for two sepreate methods: TreeMap [@page1994parallel] and ParaFit [@legendre2002statistical] (and combined) for effect sizes, papers and different levels of categorical varaibles (factors).   
...
The model (`ma_test1`), which included both random factors, had a larger AIC value (`r round(aic1,2)`) than the model with only one random effect (`r round(aic2, 2)`) . This is becuase `observation` hardly accounted any variance (< 0.0001) compared to `authors` (`r round(ma_test1$sigma2[1], 4)`). Therefore, we only had `authors` as our random factor in subsquent analyses.

We ran intercept models (meta-analyses) with 3 different datasets (`Parafit`, `TreeMap` and both combined; see the explantion of `method` above). Also, we note that we used adjustments for test statistics and confidence intervals (`test = "t"`), which is similar to (but not the same as) those proposed by Kanpp and Hartung [@knapp2003improved]; probably this approch is a more conservative approach.
...
Regression coefficients (Estimate), 95% confidence intervals (CIs), variance components (V) and variance examplained, *R*^2^~[marginal]~ [@nakagawa2013general] (R2) from the meta-regression with `symbiosis`. 
...
d <- ggplot(data = res_symbiont_tax_broad1, aes(x = tanh(estimate), y = name)) +
  ...
  scale_y_discrete(labels = c("Microbe" = "Microbe",  "Plant" = "Plant",  "Invert"= "Invertebrate",  "Vert" = "Vertebrate")) +
  annotate('text', x = 0.93, y = 1:4 + 0.15, label= paste("italic(k)==", res_symbiont_tax_broad1$n), parse=TRUE, hjust = "left", size=3.5) +
  labs(x = expression(paste(italic(r), " (correlaton)")), y = "", size = expression(paste(italic(n), " (# of species)")), tag = "d") +
  guides(fill = "none", colour = "none") +
  theme_bw() +
  theme(legend.position="none") +
  theme(axis.text.y = element_text(size = 10, colour ="black",hjust = 0.5, angle = 90)) +
...
Regression coefficients (Estimate), 95% confidence intervals (CIs), variance components (V) and variance examplained, *R*^2^~[marginal]~ (R2) from the meta-regression with `log(host_range_link_ratio)`. 
```{r}
# getting marginal R2
...
A bubble plot showing a predicted regression line for the contenious varaible `log(host_range_link_ratio)` with tehir 95% confidences regions (orange dotted lines) and 95% prediction regions (blue dotted lines) with observed effect sizes based on various sample sizes. 
...
Regression coefficients (Estimate), 95% confidence intervals (CIs), variance components (V) and variance examplained, *R*^2^~[marginal]~ (R2) from the meta-regression with `log(host_range_taxonomic_breadth)`. 
```{r}
# getting marginal R2
...
A bubble plot showing a predicted regression line for the contenious varaible `log(log(host_range_taxonomic_breadth)` with tehir 95% confidences regions (orange dotted lines) and 95% prediction regions (blue dotted lines) with observed effect sizes based on various sample sizes. 
...
fig_mode_of_transmission_broad <- ggplot(data = res_mode_of_transmission_broad1, aes(x = tanh(estimate), y = name)) +
  ...
  theme(axis.text.y = element_text(size = 10, colour ="black", hjust = 0.5, angle = 90)) +
  # adding images
  annotation_custom(rasterGrob(image_horizontal), xmin = -1, xmax = -0.8, ymin = 1.6, ymax = 2.2) +
  annotation_custom(rasterGrob(image_vertical), xmin = -1, xmax = -0.8, ymin = 2.6, ymax = 3.2) + 
  annotation_custom(rasterGrob(image_both), xmin = -1, xmax = -0.8, ymin = 0.6, ymax = 1.2) 
...
Regression coefficients (Estimate), 95% confidence intervals (CIs), variance components (V) and variance examplained, *R*^2^~[marginal]~ (R2) from the meta-regression with `symbiosis_transmission`. 
```{r}
# getting marginal R2
...
fig_symbiosis_transmission <- ggplot(data = res_symbiosis_transmission1, aes(x = tanh(estimate), y = name)) +
  ...
  # putting pictures in
  annotation_custom(rasterGrob(image_mutualism), xmin = -1.1, xmax = -0.9, ymin = 0.6, ymax = 1.2) + 
  annotation_custom(rasterGrob(image_both), xmin = -0.9, xmax = -0.7, ymin = 0.6, ymax = 1.2) + 
  annotation_custom(rasterGrob(image_mutualism), xmin = -1.1, xmax = -0.9, ymin = 1.6, ymax = 2.2) +
  annotation_custom(rasterGrob(image_horizontal),xmin = -0.9, xmax = -0.7, ymin = 1.6, ymax = 2.2) +
  annotation_custom(rasterGrob(image_mutualism), xmin = -1.1, xmax = -0.9, ymin = 2.6, ymax = 3.2) + 
  annotation_custom(rasterGrob(image_vertical), xmin = -0.9, xmax = -0.7, ymin = 2.6, ymax = 3.2) + 
  annotation_custom(rasterGrob(image_parasitism), xmin = -1.1, xmax = -0.9, ymin = 3.6, ymax = 4.2) +
  annotation_custom(rasterGrob(image_both), xmin = -0.9, xmax = -0.7, ymin = 3.6, ymax = 4.2) +
  annotation_custom(rasterGrob(image_parasitism), xmin = -1.1, xmax = -0.9, ymin = 4.6, ymax = 5.2) +
  annotation_custom(rasterGrob(image_horizontal), xmin = -0.9, xmax = -0.7, ymin = 4.6, ymax = 5.2)
...
g <- ggplot(data = res_symbiosis_transmission1, aes(x = tanh(estimate), y = name)) +
  ...
  scale_y_discrete(labels = c("MutualistBoth" = "Mutualist-\nBoth", "MutualistHorizontal" = "Mutualist-\nHorizontal","MutualistVertical" = "Mutualist-\nVertical", "ParasiteBoth" = "Parasite-\nBoth", "ParasiteHorizontal" = "Parasite-\nHorizontal")) +
  annotate('text', x = 0.93, y = 1:5 + 0.15, label= paste("italic(k)==", res_symbiosis_transmission1$n), parse=TRUE, hjust = "left", size=3.5) +
  labs(x = expression(paste(italic(r), " (correlaton)")), y = "", size = expression(paste(italic(n), " (# of species)")) ,tag = "g" ) +
  guides(fill = "none", colour = "none") +
  theme_bw() +
  theme(legend.position="none") +
  theme(axis.text.y = element_text(size = 10, colour ="black",hjust = 0.5, angle = 90)) +
...
#ggsave("../figs/fig3.png", width = 15, height = 15)
#ggsave("../figs/fig3.pdf", width = 15, height = 15)
```
**Figure 3:** putting all 7 panels together: Figure 3a - Figure 3g (see the main text)

### Addtional analyses (uni-predictor)
...
Regression coefficients (Estimate), 95% confidence intervals (CIs), variance components (V) and variance examplained, *R*^2^~[marginal]~ (R2) from the meta-regression with `host_tax_symbiosis`. 
```{r}
# getting marginal R2
...
Regression coefficients (Estimate), 95% confidence intervals (CIs), variance components (V) and variance examplained, *R*^2^~[marginal]~ (R2) from the meta-regression with `symbiont_tax_symbiosis`. 
```{r}
# getting marginal R2
...
Regression coefficients (Estimate), 95% confidence intervals (CIs), variance components (V) and variance examplained, *R*^2^~[marginal]~ (R2) from the meta-regression with `host_symbiont_tax`. 
```{r}
# getting marginal R2
...
Here we build the best model via an AICc based model selection method inplmented in the R pacakge `MuMin`[@barton2009mumin]. For the full model, we had 6 varaibles: `symbiosis`, `host_tax_broad`, `symbiont_tax_broad`, `mode_of_transmission_broad`, `endo_or_ecto`, & `log(host_range_link_ratio)`. We did not use `log(host_range_taxonomic_breadth)` as it is colinear with `log(host_range_link_ratio)` and also any of interaction terms. 
...
The averaged estimates for regression coefficients (Estimate), 95% confidence intervals (CIs), variance components (V) and variance examplained, *R*^2^~[marginal]~ (R2) from the 2 best meta-regression models.
```{r}
...
Here, we conducted 3 kinds of publication bias analyses: 1) countour-enhanced funnel plots [@peters2008contour] of residuals [@egger1997bias; @nakagawa2012methodological], 2) a type of Egger regression [@egger1997bias; @moreno2009assessment], and 3) a regression-based time-lag bias test [@nakagawa2012methodological]. 
...
It is not normal skewness that we see in our enhanced-counter funnel plot (**Supplementary Figure 5**). This funnel asymmetry is different from one cased by publication bias[@peters2008contour], but it is mainly caused by the boundary created (see "Sensitivity Analysis" section where we deal with this skewness).
...
Further, Egger regression analyses (see below) showed `sqrt(VZr)` (sampling errors [SE] for effect sizes) accounts much heteroegenity so that we added to our model. The funnel asymmetry we see in **Supplementary Figure 6** (if any) is much less sever than that in **Supplementary Figure 5**. 
...
Egger regression tests whether funnel asymmetries we see in funnel plots are statsitical signficant or not. 

#### Univaraite Egger regression

The test (or `sqrt(VZr)`) is signficant. However, as mentioned above, this is due to the boundary created by the number of randamisations; this boundary can be seen in **Supplementary Figure 7**
...
We also conducted a Egger regression controlling other important moderators (i.e., `mode_of_transmission_broad`, `host_tax_broad`, & `symbiosis`). After controlling for these variables, `sqrt(VZr)` stays signficant.
...
A bubble plot showing a predicted loess line for the contenious varaible `log(host_range_link_ratio)` (given the values fo the other 3 varaibles in the model) with tehir 95% confidences regions (orange dotted lines) and 95% prediction regions (blue dotted lines) with observed effect sizes based on various sample sizes. Note that the lines are not linear as these are based on multivraite predctions of the data points. 

### Time-lag bias

We do not find any evidence of the time-lag effect (a decline in the magnitude of the effect over time) in both the univaraite and multivariate models (**Supplementary Figure 9** and **Supplementary Figure 10**).
...
Regression coefficients (Estimate), 95% confidence intervals (CIs), variance components (V) and variance examplained, *R*^2^~[marginal]~ (R2) from the meta-regression with `year`. 
```{r}
# getting marginal R2
...
**Supplementary Figure 10:** A bubble plot showing a predicted loess line for the contenious varaible `year` (given the values fo the other 3 varaibles in the model) with tehir 95% confidences regions (orange dotted lines) and 95% prediction regions (blue dotted lines) with observed effect sizes based on various sample sizes. Note that the lines are not linear as these are based on multivraite predctions of the data points. 

## Sensitivity Analysis

The funnel plots above identifed the issue of upper bounds of this effect size given a sample size (a upper limite of a p value given the number of randomaizations). There are two analyses, we can do to confirm that this boundary is not affecting our overall results. First, 

important moderators (i.e., `mode_of_transmission_broad`, `host_tax_broad`, & `symbiosis`)
...
* Researchers need to set the number of randamizations much higher to obtain more accurate p value. 
* If possible, the reanalysis of earlier trees are necessary; also, analysis using updated trees will be desierable.
* New studies should provide all data avaiable online so that a future meta-analysis can incorprate all avaiable data.

## Acknowledgement

Many coding materials have been borrowed from Luke's paper. 
