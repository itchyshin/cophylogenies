...
In addtion to these, we ran three more univariate models: 1) `host_tax_symbiosis` (equivalent to the interactoin term between `symbiosis` and `host_tax_symbiosis`; `symbiosis*host_tax_symbiosis`), 2) `symbiont_tax_symbiosis` (`symbiosis*symbiont_tax_broad`) and 3) `host_symbiont_tax` (`host_tax_symbiosis*symbiont_tax_broad`). There moderators are created below:
...
pander(tibble(
  ...
  `R2 %` = c(r2_mode_of_transmission_broad1[1], rep(NA, 5))), 
  # split.table = Inf stops spliting - also text gets wrapped
  split.table = Inf, digits = 3)
```
...
fig_mode_of_transmission_broad <- ggplot(data = res_mode_of_transmission_broad1, aes(x = tanh(estimate), y = name)) +
  ...
  geom_quasirandom(data = dat %>% filter(!is.na(mode_of_transmission_broad)), 
                   aes(x= tanh(Zr), y = mode_of_transmission_broad, size = 1/VZr, colour = mode_of_transmission_broad), groupOnX = FALSE, alpha=0.4) + 
  # 95 %precition interval (PI)
  geom_errorbarh(aes(xmin = tanh(lowerPR), xmax = tanh(upperPR)),  height = 0, show.legend = F, size = 0.5, alpha = 0.6) +
  # 95 %CI
  geom_errorbarh(aes(xmin = tanh(lowerCL), xmax = tanh(upperCL)),  height = 0, show.legend = F, size = 1.2) +
...
pander(tibble(
  ...
  `R2 %` = c(r2_endo_or_ecto1[1], rep(NA, 5))), 
  # split.table = Inf stops spliting - also text gets wrapped
  split.table = Inf, digits = 3)
```
...
fig_endo_or_ecto <- ggplot(data = res_endo_or_ecto1, aes(x = tanh(estimate), y = name)) +
  ...
  geom_quasirandom(data = dat %>% filter(!is.na(endo_or_ecto)), 
                   aes(x= tanh(Zr), y = endo_or_ecto, size = 1/VZr, colour = endo_or_ecto), groupOnX = FALSE, alpha=0.4) + 
  # 95 %precition interval (PI)
  geom_errorbarh(aes(xmin = tanh(lowerPR), xmax = tanh(upperPR)),  height = 0, show.legend = F, size = 0.5, alpha = 0.6) +
  # 95 %CI
  geom_errorbarh(aes(xmin = tanh(lowerCL), xmax = tanh(upperCL)),  height = 0, show.legend = F, size = 1.2) +
  ...
  scale_y_discrete(labels = c("Endo/Ecto" = "Both",  "Endo" = "Horizontal",  "Ecto"= "Vertical")) +
...
pander(tibble(
  ...
  `R2 %` = c(r2_host_tax_symbiosis1[1], rep(NA, (8 + choose(8,2))- 1))), 
  # split.table = Inf stops spliting - also text gets wrapped
  split.table = Inf, digits = 3)
```
...
fig_host_tax_symbiosis <- ggplot(data = res_host_tax_symbiosis1, aes(x = tanh(estimate), y = name)) +
  ...
  geom_quasirandom(data = dat %>% filter(!is.na(host_tax_symbiosis)), 
                   aes(x= tanh(Zr), y = host_tax_symbiosis, size = 1/VZr, colour = host_tax_symbiosis), groupOnX = FALSE, alpha=0.4) + 
  # 95 %precition interval (PI)
  geom_errorbarh(aes(xmin = tanh(lowerPR), xmax = tanh(upperPR)),  height = 0, show.legend = F, size = 0.5, alpha = 0.6) +
  # 95 %CI
  geom_errorbarh(aes(xmin = tanh(lowerCL), xmax = tanh(upperCL)),  height = 0, show.legend = F, size = 1.2) +
...
pander(tibble(
  ...
  `R2 %` = c(r2_host_tax_symbiosis1[1], rep(NA, (7 + choose(7,2))- 1))), 
  # split.table = Inf stops spliting - also text gets wrapped
  split.table = Inf, digits = 3)
```
...
fig_host_tax_broad <- ggplot(data = res_host_tax_broad1, aes(x = tanh(estimate), y = name)) +
  ...
  geom_point(aes(fill = name), size = 3, shape = 21) + #
  # setting colours
  scale_color_manual(values = c("InvertMutualist" = colour_ls[1],   "InvertParasite"= colour_ls[2], "MicrobeMutualist"= colour_ls[3], "MicrobeParasite"= colour_ls[4],  "PlantMutualist"= colour_ls[5], "PlantParasite"= colour_ls[6], "VertMutualist"= colour_ls[7],     "VertParasite"= colour_ls[8] )) +
  scale_fill_manual(values = c("InvertMutualist" = colour_ls[1],   "InvertParasite" = colour_ls[2], "MicrobeMutualist"= colour_ls[3], "MicrobeParasite"= colour_ls[4],  "PlantMutualist"= colour_ls[5], "PlantParasite"= colour_ls[6], "VertMutualist"= colour_ls[7],     "VertParasite"= colour_ls[8] )) +
  scale_y_discrete(labels = c("InvertMutualist" = "Invertebrate- \nMutualist",   "InvertParasite"= "Invertebrate- \nMutualist", "MicrobeMutualist"= "Microbe- n\Mutualist", "MicrobeParasite"= "Microbe- n\Parasite",  "PlantMutualist" = "Plant- \nMutualist", "PlantParasite"="Plant- \nParasite", "VertMutualist"= "Vertebrate- \nMutualist",     "VertParasite"= "Vertebrate- \nParasite" )) +
  annotate('text', x = -1, y = res_host_tax_broad1$name, label= paste("italic(k)==", res_host_tax_broad1$n), parse=TRUE, hjust = "left", size=3.5) +
  labs(x = expression(paste(italic(r), " (correlaton)")), y = "", size = expression(paste(italic(n), " (# of species)")) ) +
  guides(fill = "none", colour = "none") +
  theme_bw() +
