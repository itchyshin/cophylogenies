...
res_range_link_ratio <- get_est(mr_host_range_link_ratio, mod = "log(host_range_link_ratio)")

# creating a table
tibble(
  `Fixed effect` = c("Intercept", "log(host_range_link_ratio)"),
  Estimate = c(res_range_link_ratio$estimate),
  `Lower CI [0.025]` = c(res_range_link_ratio$lowerCL),
  `Upper CI  [0.975]` = c(res_range_link_ratio$upperCL),
...
```{r, fig.width=7, fig.height= 4}
# getting CI for regression line
...
# "Invert"= "#0072B2",  "Vert" = "#D55E00"

# plotting
dat %>% 
  filter(!is.na(host_range_link_ratio)) ->dat_temp
fig_host_range_link_ratio <-  # getting ride of NA values
  ggplot(data = dat_temp, aes(x = log(host_range_link_ratio), y = Zr, size = (1/VZr) + 3, )) +
  geom_point(shape = 21, fill = "grey90") +
  geom_abline(intercept = mr_host_range_link_ratio$beta[[1]], slope = mr_host_range_link_ratio$beta[[2]], alpha = 0.7, linetype = "dotted", size = 1) +
  labs(x = "ln(range link ratio)", y = expression(paste(italic(Zr), " (effect size)")), size = expression(paste(italic(n), " (# of species)"))) +
  guides(fill = "none", colour = "none") +
  geom_ribbon(data = ribbon_dat, aes(ymin = ymin, ymax = ymax, x = newmods), fill = "#0072B2")  +
  # themses
  theme_bw() +
...
fig_host_range_link_ratio2 #<- fig_host_range_link_ratio + geom_ribbon(data = ribbon_dat, aes(ymin = ymin, ymax = ymax, x = newmods),fill = "#0072B2")
```
