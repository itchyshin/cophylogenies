...
# eq <- substitute(italic(Sepal.Length) == a - b %*% italic(Petal.Length)~","~italic(R)^2~"="~r2~","~italic(P)~"="~p, d)
# annotate('text', 7, 4, label=paste("italic(Sepal.Length)==", d$a, "~-~", d$b, "~x~", "~italic(Petal.Length)~';'~italic(r)^2==", d$r2, "~italic(P)==", d$p), parse=TRUE, hjust=1, size=3.5)

#xxx
effect_ma <- get_est(ma_all)
fig_ma <- ggplot(data = effect_ma, aes(x = esr, y = "Overall mean")) +
  scale_x_continuous(limits=c(-1, 1), breaks = seq(-1, 1, by = 0.2) ) +
  geom_errorbarh(aes(xmin = lcir, xmax = ucir),  height = 0, show.legend = F, size = 1) +
  ...
  # it is difficult to mix italitc and r code in geom_text (annoate will be easier)
  #geom_text(data = dat, aes( x = -1, y = "Overall mean", label = str_c("k = ", length(Zr)) ), size = 3, hjust = "left") +
  #dgeom_text(data = dat, aes( x = -1, y = "Overall mean", label = "italic('k =')~233", ), size = 3, hjust = "left", parse = TRUE) +
...
#ggsave(plot = fig_ma, filename = "fig_1a.pdf", height = 2, width = 8)
