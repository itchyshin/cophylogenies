...
```{r, fig.width=7, fig.height= 4}
fig_host_range_link_ratio <- ggplot(dat, aes(x = host_range_link_ratio, y = Zr, size = (1/VZr) + 3)) +
    geom_point(shape = 21) +
    geom_abline(intercept = mr_host_range_link_ratio$beta[[1]], slope = mr_host_range_link_ratio$beta[[2]], 
                alpha = 0.7, linetype = "dotted", size = 1.5) +
    labs(x = "ln(range link ratio)", y = expression(paste(italic(Zr), " (effect size)")))
