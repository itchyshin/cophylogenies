...
X. __method__:	Whether TreeMap or ParaFit was used to obtain the reported p value.
...
dat$analyses <- paste0(dat$authors, dat$host_tax_fine, dat$symbiont_tax_fine, dat$total_host_symbioint_links) # the number of analyses (studies)
n_analyses <- n_distinct(dat$analyses)
n_table2 <-t(n_table1[,-1])
colnames(n_table2) <- n_table1$method
n_table2 %>% as_tibble(rownames = "Number") %>% 
  mutate(Combined = Parafit + TreeMap, Combined = replace(Combined, c(2,3), c(n_analyses, n_authors))) %>%  
...
```
Note that for the numbers of analyses and papers does not add up (TreeMap + ParaFit $\neq$ Combined), because `r sum(n_table2[2,]) - length(unique(dat$analyses))` analyses and `r sum(n_table2[3,]) - length(unique(dat$authors))`  papers used both the TreeMap and ParaFit methods (the term "papers" here is our variable `authors`)
...
dat %<>% 
  ...
  mutate(., sample_size = host_tips_linked_corrected + symbiont_tips_linked, 
...
# sum(dat$sample_size[match(unique(dat$analyses), dat$analyses)])
```
...
Below, we show that categories with higher effect sizes were more likely to have "bounded" effect sizes (`limit_rearched`) in all `mode_of_transmission_broad`, `host_tax_broad`, & `symbiosis`. This indicates that the higher the estimate of mean effect size is, the more underestimated the mean effect size is. This is true for differences between two categories; the larger the difference between two, the more underestimated the difference is. 


#### The type of symbiosis: parasitism vs. mutualism

**Supplementary Table 22:**
Regression coefficients (estimate), 95% confidence intervals (CIs), variance components (V) and variance explained, *R*^2^~[marginal]~ (R2) from the meta-regression with `symbiosis` (compare the effect estimates in **Supplementary Table 2** with the corresponding values in this table). 
...
Regression coefficients (estimate), 95% confidence intervals (CIs), variance components (V) and variance explained, *R*^2^~[marginal]~ (R2) from the meta-regression with `mode_of_transmission_broad` (compare the effect estimates in **Supplementary Table 8** with the corresponding values in this table).
