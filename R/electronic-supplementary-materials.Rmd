...
dat %>% summarise_all(funs(sum(is.na(.)))) %>% # map(~sum(is.na(.)) # this is an alterantive way 
...
dat %>% 
  ...
  p_to_Zr(p_value, sample_size) %>% 
  # observation level random effect
  mutate(obs = factor(1:nrow(dat))) -> dat 
...
main_model_all <- rma.mv(yi = Zr, V = VZr, mods = ~ symbiosis, random = list(~ 1 | authors, ~1|obs), data = dat)

summary(main_model_all)


main_model_all2 <- rma.mv(yi = Zr, V = VZr, mods = ~ symbiosis, random = ~ 1 | authors, data = dat)

summary(main_model_all2)
