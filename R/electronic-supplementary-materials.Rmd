...
```
Note that for the numbers of analyses and papers does not add up (TreeMap + ParaFit $\neq$ Combined), because `r sum(n_table2[3,]) - length(unique(dat$authors))` analyses and `r sum(n_table2[3,]) - length(unique(dat$authors))`  papers used both the TreeMap and ParaFit methods (the term "papers" here is our variable `authors`)
