...
#### Testing the effect of the type of symbiosis: parasitism vs. mutualism
...
#### Testing the effect of host taxa
```{r}
# reordering
...
fig_host_tax_broad <- ggplot(data = res_host_tax_broad1, aes(x = tanh(estimate), y = name)) +
  ...
  geom_quasirandom(data = dat %>% filter(!is.na(host_tax_broad)), 
                   aes(x= tanh(Zr), y = host_tax_broad, size = 1/VZr, colour = host_tax_broad), groupOnX = FALSE, alpha=0.2) + 
  # 95 %precition interval (PI)
  geom_errorbarh(aes(xmin = tanh(lowerPR), xmax = tanh(upperPR)),  height = 0, show.legend = F, size = 0.5, alpha = 0.6) +
  # 95 %CI
  geom_errorbarh(aes(xmin = tanh(lowerCL), xmax = tanh(upperCL)),  height = 0, show.legend = F, size = 1.2) +
  ...
  geom_point(aes(fill = name), size = 3, shape = 21) + #
  # setting colours
  scale_color_manual(values = c("Microbe" = "#000000",  "Plant" = "#E69F00",  "Invert"= "#56B4E9",  "Vert" = "#009E73")) +
  scale_fill_manual(values = c("Microbe" = "#000000",  "Plant" = "#E69F00",  "Invert"= "#56B4E9",  "Vert" = "#009E73")) +
...
#### Testing the effect of symbiont taxa
...
fig_symbiont_tax_broad <- ggplot(data = res_symbiont_tax_broad1, aes(x = tanh(estimate), y = name)) +
  ...
  geom_quasirandom(data = dat %>% filter(!is.na(symbiont_tax_broad)), 
                   aes(x= tanh(Zr), y = symbiont_tax_broad, size = 1/VZr, colour = symbiont_tax_broad), groupOnX = FALSE, alpha=0.2) + 
  # 95 %precition interval (PI)
  geom_errorbarh(aes(xmin = tanh(lowerPR), xmax = tanh(upperPR)),  height = 0, show.legend = F, size = 0.5, alpha = 0.6) +
  # 95 %CI
  geom_errorbarh(aes(xmin = tanh(lowerCL), xmax = tanh(upperCL)),  height = 0, show.legend = F, size = 1.2) +
...
#### Testing the effect of the model of transmission

#### Testing the effect of the type of symbiosis: endosymbiosis vs. ectosymbiosis
