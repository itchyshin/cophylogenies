...
n_analyses <- n_distinct(paste0(dat$authors, dat$host_tax_broad, dat$symbiont_tax_broad, dat$method))
...
```
Note that for the numbers of analyses and papers does not add up (TreeMap + ParaFit $\neq$ Combined), because `r sum(n_table2[2,]) - length(unique(dat$authors))` analyses and `r sum(n_table2[3,]) - length(unique(dat$authors))`  papers used both the TreeMap and ParaFit methods (the term "papers" here is our variable `authors`)
...
dat %<>% 
  ...
  mutate(., sample_size = host_tips_linked_corrected + symbiont_tips_linked, 
         ...
         analysis = factor(paste0(dat$authors, dat$host_tax_broad, dat$symbiont_tax_broad, dat$method))
         )  
