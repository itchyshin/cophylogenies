...
fig_ma <- ggplot(data = effect_ma, aes(x = esr, y = "Overall mean")) +
  ...
  #geom_text(data = dat, aes( x = -1, y = "Overall mean", label = "italic('k =')~233", ), size = 3, hjust = "left", parse = TRUE) +
...
We ran a univaraite meta-regression model for each of following moderators: 1) `symbiosis`, 2) `symbiont_tax_broad`, 3) `host_tax_symbiosis`, 4) `host_range_link_ratio`, 5) `host_range_taxonomic_breadth`, 6) `mode_of_transmission_broad`, and 7) `endo_or_ecto`. The results from these models are mentioned in the main text. In addtion to these, we ran three more univariate models: 1) , 2) and 3) .

NOTE - making interaction terms
NOTE - Reording - factors!!!
