...
full_data <- read_csv("../data/2019-04-01-source-data-dat.csv", na = "NA") %>% 
...
We present the number of sample size for two sepreate methods: TreeMap and Parafit (and combined) for effect sizes, papers and different levels of categorical varaibles (factors). Note that for the numbers of papers do not add up (TreeMap + Parafit $\neq$ Combined) becuase 15 papers used both TreeMap and Parafit methods.  

```{r}
# selecting out variables, which we used for our analysis
data <- full_data %>% select(-host_tax_fine, -symbiont_tax_fine, -symbiont_euk, -mode_of_transmission_fine, -symbiont, -`Visiting_symbiont?`)

# making a table of sample sizes for different variables
n_authors <- n_distinct(data$authors) # the total number of papers
...
# transposing the table and creating that table
n_table2 <-t(n_table1[,-1])
colnames(n_table2) <- n_table1$method
n_table2 %>% as_tibble(rownames = "Number") %>% 
  mutate(Combined = Parafit + TreeMap, Combined = if_else(Combined == Combined[2], n_authors, Combined)) %>%    
...
data %>% summarise_all(funs(sum(is.na(.)))) %>% # map(~sum(is.na(.)) # w
