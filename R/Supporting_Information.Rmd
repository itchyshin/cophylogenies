...
tibble(
  ...
  `R2` = c(r2_host_tax_broad1[1], rep(NA, 13))) %>% kable("html", digits = 3) %>%
  kable_styling("striped", position = "left") %>%
...
tibble(
  ...
  `R2` = c(r2_mode_of_transmission_broad1[1], rep(NA, 8))) %>% kable("html", digits = 3) %>%
  kable_styling("striped", position = "left") 
```
...
fun1 <- funnel(res_funnel_plot, yaxis = "seinv", level=c(90, 95, 99), 
...
fun2 <- funnel(res_funnel_plot2, yaxis = "seinv", level=c(90, 95, 99),
       xlim  = c(-3, 4), ylim = c(0.9, 5.2), xlab = "Residuals (correlation)", ylab = "Precision (1/SE)",
...
#### Table S5.4
Regression coefficients (estimate), 95% confidence intervals (CIs), variance components (V) and variance explained, *R*^2^~[marginal]~ (R2) from the regression with `symbiosis` on `limit_reached` (compare the effect estimates in **XXXX** with the corresponding values in this table). 
...
#### Table S5.5
Regression coefficients (estimate), 95% confidence intervals (CIs), variance components (V) and variance explained, *R*^2^~[marginal]~ (R2) from the regression with `host_tax_broad` on `limit_reached` (compare the effect estimates in **XXXX** with the corresponding values in this table). 
...
#### Table S5.6
Regression coefficients (estimate), 95% confidence intervals (CIs), variance components (V) and variance explained, *R*^2^~[marginal]~ (R2) from the regression with `mode_of_transmission_broad` on `limit_reached` (compare the effect estimates in **XXXX** with the corresponding values in this table).
