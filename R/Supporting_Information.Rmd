...
There are strong and significant correlations in effect sizes (Zr) between `ParaFit` and `TreeMap` (for all the 16 studies, r = `r `, t = , df = , p = and for the 11 perfectly matching studies, r = r , t = , df = , p = ). Therefore, we have made an assumption that effect sizes based on p values from these two methods can be equated in our analyses below. 
...
A normal funnel plot assumes homogeneity (i.e., *I*^2^ = 0). Therefore, we controlled for important moderators (i.e., `mode_of_transmission_broad`, `host_tax_broad`, & `symbiosis`).
...
A bubble plot showing a predicted loess line for the contentious variable `log(host_range_link_ratio)` (given the values of the other 3 variables in the model), with their 95% confidence regions (orange dotted lines) and 95% prediction regions (blue dotted lines) with observed effect sizes based on various sample sizes. Note that the lines are not linear as these are based on multivariate predictions of the data points. 
